...
SESSION_COOKIE_NAME = "res_session"


@router.post("/login")
def login(
    body: LoginIn, response: Response, use_case: LoginUser = Depends(get_login_user)
) -> dict:
    try:
...
def get_session(ctx: ActingContext = Depends(get_acting_context)) -> SessionOut:
    ...
    return SessionOut(user_id=ctx.user_id, project_id=ctx.project_id, role=ctx.role)
