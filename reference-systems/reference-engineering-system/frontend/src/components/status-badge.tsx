...
const DESIGN_CHANGE_STATUS_STYLES: Record<string, string> = {
  DRAFT: "bg-muted text-muted-foreground",
  ISSUED: "bg-blue-100 text-blue-900 dark:bg-blue-950 dark:text-blue-300",
  ACKNOWLEDGED: "bg-emerald-100 text-emerald-900 dark:bg-emerald-950 dark:text-emerald-300",
  SUPERSEDED: "bg-muted text-muted-foreground line-through decoration-1",
  VOID: "bg-red-100 text-red-900 dark:bg-red-950 dark:text-red-300",
};

export function DesignChangeStatusBadge({ status }: { status: string }) {
  return (
    <Badge
      variant="outline"
      className={cn(
        "border-transparent font-medium",
        DESIGN_CHANGE_STATUS_STYLES[status] ?? "bg-muted"
      )}
    >
      {status}
    </Badge>
